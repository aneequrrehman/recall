...
describe('sqliteAdapter', () => {
  ...
  beforeEach(() => {
    ...
    adapter = sqliteAdapter({ filename: TEST_DB })
  })
  ...
  describe('queryByEmbedding', () => {
    it('returns memories sorted by similarity', async () => {
      ...
      await adapter.insert({
        ...
        embedding: [0.9, 0.1, 0.1], // Most similar to query
        metadata: {},
      })
      await adapter.insert({
        userId: 'user-1',
      ...
      await adapter.insert({
        userId: 'user-1',
        content: 'Not similar',
        embedding: [0.1, 0.9, 0.1], // Least similar
        metadata: {},
      })
    ...
    it('only returns memories for the specified user', async () => {
      ...
      await adapter.insert({
        ...
        embedding: [0.95, 0.05, 0.05], // Even more similar but different user
        metadata: {},
      })
  ...
  describe('in-memory mode', () => {
    it('works with :memory: filename', async () => {
      const memAdapter = sqliteAdapter({ filename: ':memory:' })

      const memory = await memAdapter.insert({
        userId: 'user-1',
      ...
      const retrieved = await memAdapter.get(memory.id)
      expect(retrieved).not.toBeNull()
      expect(retrieved!.content).toBe('In-memory test')
    })
  })
})
